...
class Num {
  constexpr void reduce() { ((i >= max) || (i < 0)) && (i = 0); }
...
int i = iter++;
    if (i >= constants.size[arr_w[track3]])
    {
      track3++;
      if(arr_w[track3] == 0 )continue
      iter.exchange(0);
      i = 0;
    }
  ...
  void snapValues(int u, int i, int thread_id) {
    // i %= constants.size[u]-1;

    // if(  constants.outDegree[u].size() != constants.size[u]){
    //   int v = constants.outDegree[u].size();
    //     int c = constants.size[u];
    // }
    
    if (i == constants.outDegree[arr_w[track3]].size() || arr_w[track3]==0) {
      
      pthread_mutex_unlock(&binary_lock);
      return;
    }

    auto next = constants.outDegree[arr_w[track3]][i];
    int v = 0;
    int w = 0;
    v = next.first;
    w = next.second;
    ...
    constants.d[v] = constants.d[arr_w[track3]] + w;
