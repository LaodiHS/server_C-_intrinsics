...
class Solution : public Buffer<int, 900, 100> {
 public:
  ...
  if (track3 > track) {
    block = true;
         pthread_mutex_unlock(&binary_lock2);
    pthread_exit(&thread_pool[thread_id]);
      ...
      int i = iter  + thread_id ;

      int s = constants.size[u];
      if (i > s)
      {

        pthread_mutex_lock(&binary_lock2);

        // break;
      };

      if (i-1 == constants.size[u]) {
  ...
  void snapValues(int u, int i, int thread_id) {
    ...
    if (constants.d[arr_w[track3]] + w >= constants.d[v]) {
